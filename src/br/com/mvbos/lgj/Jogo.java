		...
		while (true) {
			if (System.currentTimeMillis() >= prxAtualizacao) {
				...
				if (controleTecla[Tecla.BA.ordinal()]) {
					...
					}else if (cenario instanceof FimCenario) {
						cenario.descarregar();
						cenario = null;
						cenario = new RankingCenario(tela.getWidth(), tela.getHeight());
				...
				if(cenario instanceof JogoCenario) {
					if(JogoCenario.getEstado() == JogoCenario.Estado.PERDEU || 
							JogoCenario.getEstado() == JogoCenario.Estado.GANHOU) {
						cenario.descarregar();
						cenario = null;
